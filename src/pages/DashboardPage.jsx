...
import { Plus, LogOut, Hash, Users, ArrowRight, Loader2, X, FolderOpen } from 'lucide-react';
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            fontSize:'20px', fontWeight:600, color:'#f0ede8', letterSpacing:'0.06em',
          }}>{title}</span>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'20px' }}>
        <div>
          ...
          <input
            ...
            placeholder="e.g. Product Sprint" maxLength={40} style={inputStyle}
          ...
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
            {COLORS.map(c => (
              <button key={c} type="button" onClick={() => setColor(c)} style={{
                ...
                background:c, border:'none', cursor:'pointer', transition:'all 0.15s',
        ...
        <button
          onClick={() => { if (name.trim()) mutation.mutate({ name: name.trim(), icon, color }); }}
    ...
    >
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'16px' }}>
        <div style={{
          ...
          background: ws.color + '18', border:`1px solid ${ws.color}30`,
        }}>
          {ws.icon}
        </div>
        <ArrowRight style={{
          width:16, height:16,
          color: hovered ? '#e8a24a' : 'rgba(240,237,232,0.2)',
          transform: hovered ? 'translateX(3px)' : 'none',
          transition:'all 0.2s',
        }} />
      </div>
      <p style={{
        fontSize:'15px', fontWeight:500, color:'#f0ede8', marginBottom:'6px',
        overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', margin:'0 0 6px',
      }}>{ws.name}</p>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0b0b0c', fontFamily:"'DM Sans',sans-serif", color:'#f0ede8', position:'relative', overflow:'hidden' }}>
      ...
      <header style={{
        ...
        <a href="/" style={{
          fontFamily:"'Cormorant Garamond',serif",
          fontSize:'20px', fontWeight:600, letterSpacing:'0.12em',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'20px' }}>
          ...
          <button onClick={handleLogout} style={{
            ...
            cursor:'pointer', fontFamily:'inherit', fontWeight:500, transition:'color 0.2s',
          }}
      ...
      <main style={{ position:'relative', zIndex:10, maxWidth:'1000px', margin:'0 auto', padding:'60px 48px' }}>
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:'48px', flexWrap:'wrap', gap:'20px' }}>
          <div>
            <span style={{ fontSize:'10px', letterSpacing:'0.3em', color:'#e8a24a', fontWeight:500, display:'block', marginBottom:'10px' }}>
              YOUR WORKSPACES
            </span>
            <h1 style={{
              fontFamily:"'Cormorant Garamond',serif",
              fontSize:'clamp(32px,4vw,48px)', fontWeight:300,
              color:'#f0ede8', lineHeight:1.1, margin:0,
            }}>
          ...
          <div style={{ display:'flex', gap:'10px' }}>
            <button onClick={() => setModal('join')} style={{
              display:'flex', alignItems:'center', gap:'6px',
              padding:'10px 20px', fontSize:'12px', fontWeight:500, letterSpacing:'0.06em',
            ...
            >
              <Hash style={{ width:13, height:13 }} /> JOIN
            </button>
            <button onClick={() => setModal('create')} style={{
              display:'flex', alignItems:'center', gap:'6px',
              padding:'10px 20px', fontSize:'12px', fontWeight:600, letterSpacing:'0.08em',
              color:'#0b0b0c', background:'#e8a24a', border:'none',
        ...
        {isLoading ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'16px' }}>
            {[...Array(3)].map((_,i) => (
              <div key={i} style={{ height:'130px', borderRadius:'2px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.05)' }} />
            ))}
          </div>
        ) : workspaces.length === 0 ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'80px 0', gap:'16px', textAlign:'center' }}>
            ...
            <div style={{ display:'flex', gap:'10px', marginTop:'8px' }}>
              <button onClick={() => setModal('join')} style={{ padding:'10px 20px', fontSize:'12px', letterSpacing:'0.06em', color:'rgba(240,237,232,0.6)', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'2px', cursor:'pointer', fontFamily:'inherit', fontWeight:500 }}>
                JOIN WITH CODE
              </button>
              <button onClick={() => setModal('create')} style={{ padding:'10px 20px', fontSize:'12px', letterSpacing:'0.08em', color:'#0b0b0c', background:'#e8a24a', border:'none', borderRadius:'2px', cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>
                CREATE WORKSPACE
              </button>
            </div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'16px' }}>
            {workspaces.map(ws => (
              <WorkspaceCard key={ws._id} ws={ws} onClick={() => navigate(`/workspace/${ws._id}`)} />
            ))}
            <button onClick={() => setModal('create')} style={{
